...
class Trainer_rectangle_loss(nnUNetTrainer_interactive.nnUNetTrainerinteractive):
    """custom nnUNet Trainer -> only difference is that the second term of the loss is not a sphere around the click but a big stupid cube.
    note : click simulation is force to be redesign to get a list of all the clicks ..."""
    ...
    def _build_loss(self,):
        ...
        loss=Mutils.rectangle_loss({'batch_dice': self.configuration_manager.batch_dice,
                                   'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, {'ignore_index':self.label_manager.ignore_label}, weight_ce=1, weight_dice=1,
                                  ignore_label=self.label_manager.ignore_label, dice_class=MemoryEfficientSoftDiceLoss)
