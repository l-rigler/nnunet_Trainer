...
class Trainer_alpha_0(nnUNetTrainer_interactive.nnUNetTrainerinteractive):
    """custom nnUNet Trainer -> only difference is that the second term of the loss is not a sphere around the click but a big stupid cube.
    note : click simulation is force to be redesign to get a list of all the clicks ..."""
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with torch.autograd.set_detect_anomaly(True) : # FOR DEBUGGING ONLY
            with (
                ...
                output = self.network(data)
                if self.current_epoch> 0 :
                    self.loss.click_map = None
                    self.loss.loss.alpha = 1
                l=self.loss(output,target)
            if self.grad_scaler is not None:
                self.grad_scaler.scale(l).backward()
                self.grad_scaler.unscale_(self.optimizer)
...
class Trainer_alpha_0_2(nnUNetTrainer.nnUNetTrainer):
